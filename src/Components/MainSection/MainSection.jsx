...
const MainSection = () => {
    let [ipQuerry,setIpQuerry]=useState('');
    // console.log(ipQuerry);
    
    return (
       <main className='main-box'>
       <h1 className='main-heading'>Search Your Restaurant</h1>
       <div className='ip-box'>
       <div className='ip-name-box'>
       <label htmlFor='name'>Enter Name:-</label>
        <input type="text"  id='name' className='ip ip-name' value={ipQuerry} 
        onChange={(e)=>{setIpQuerry(e.target.value)}}/>
       </div>
      <div className='ip-rating-box'>
      <label htmlFor='rating'>Minimum Rating:-</label>
        <input type="number" id='rating' className='ip ip-rating' />
      </div>
       </div>
       <section className='card-box'>
          {
            data.filter((item)=>{
                return item.name.toLowerCase().includes(ipQuerry.toLowerCase())
            }).map((item,idx)=>{
               return(
                <div className='card' key={idx}>
                <div className='card-top'>
                    <p className='restro-name'>{item.name}</p>
                    <p className='restro-address'>
                        <img src="https://static.vecteezy.com/system/resources/previews/015/311/773/non_2x/red-location-pin-icon-transparent-background-free-vector.jpg" alt="" width="30px"/>
                        {item.address}
                    </p>
                    <p className='restro-codes'><span>{item.outcode}</span><span>{item.postcode}</span></p>
                </div>
                <div className='card-bot'>
                    <p className='restro-type'>{item.type_of_food}</p>
                    <p className='visit-menu'><a href={item.URL}>Visit Menu</a></p>
                </div>
                </div>
               )
            })
